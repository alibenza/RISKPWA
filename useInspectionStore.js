// src/stores/useInspectionStore.js
import { create } from 'zustand';
import { persist, createJSONStorage } from 'zustand/middleware';
import { get, set, del, keys } from 'idb-keyval';

// ==========================================
// DEBOUNCED STORAGE IMPLEMENTATION
// ==========================================
class DebouncedStorage {
  ...
  setItem(name, value) {
    ...
    // Debounce write by 800ms
    const timeout = setTimeout(async () => {
      try {
        const dataToStore = this.pending.get(name);
        await set(name, dataToStore);
        this.pending.delete(name);
        console.log(`[Storage] Auto-saved: ${name}`);
      } catch (error) {
        console.error(`[Storage] Save error for ${name}:`, error);
      } finally {
  ...
  // Force immediate save (use before export/unload)
  async flush() {
    const promises = [];
    for (const [name, value] of this.pending) {
      promises.push(set(name, value).then(() => {
        console.log(`[Storage] Flushed: ${name}`);
      }));
    }
...
const idbStorage = {
  getItem: (name) => debouncedStorage.getItem(name),
  setItem: (name, value) => debouncedStorage.setItem(name, value),
  removeItem: (name) => debouncedStorage.removeItem(name),
};
...
// Deep clone using structuredClone (fallback to JSON)
const deepClone = (obj) => {
...
const DEFAULT_SITE = {
  ...
  photos: {} // { qId: [{ photoId, timestamp, description, size }] }
};
...
export const useInspectionStore = create(
  persist(
    (set, get) => ({
      ...
      allSites: {
        'Site_Initial': deepClone(DEFAULT_SITE)
      },
      
      // Compatibility state (mirrors active site)
      responses: {},
      aiResults: null,
      chatHistory: [],
      smpData: deepClone(DEFAULT_SMP_DATA),
      ...
      history: [], // Global history (limited size)
      isLoaded: false,

      // --- COMPUTED GETTERS ---
      
      /**
       * Get current active site data
       */
      getCurrentSite: () => {
        const state = get();
      ...
      /**
       * Get all alerts (low scores) for current site
       */
      getAlerts: () => {
        const state = get();
      ...
      /**
       * Switch between sites with proper data isolation
       */
      switchSite: (siteId, name = null) => {
        const state = get();
        
        // Deep clone all sites to prevent reference sharing
        const updatedAllSites = deepClone(state.allSites);
        
        // Save current site state before switching
        ...
        console.log(`[Store] Switched to site: ${siteId}`);
      },
      ...
      /**
       * Delete site with confirmation safeguard
       */
      deleteSite: (siteId) => {
        const state = get();
        ...
        // Also clean up photo blobs
        const sitePhotos = state.allSites[siteId]?.photos || {};
      ...
      /**
       * Set response with auto-save to current site
       */
      setResponse: (id, data) => set((state) => {
        ...
        // Async update to allSites (debounced)
        setTimeout(() => {
      ...
      /**
       * Batch update responses (for imports)
       */
      ...
      /**
       * Set AI results with history tracking
       */
      setAiResults: (results) => {
        ...
        // Add to history if valid results
        if (results && results.score_global) {
          ...
          set((s) => ({
            history: [historyEntry, ...s.history.slice(0, 49)] // Keep last 50
          }));
        }
      },

      /**
       * Chat message with size limit
       */
      ...
      /**
       * Photo management with external blob storage
       */
      addPhoto: async (qId, file, description = '') => {
        const photoId = `photo_${generateId()}`;
        
        // Store blob in IndexedDB directly (not in Zustand)
        try {
          ...
          set((state) => {
            ...
            // Update site with photo metadata only
            setTimeout(() => {
              const s = get();
            ...
            // Also update responses for backward compatibility
            const newResponses = {
              ...state.responses,
              [qId]: {
                ...state.responses[qId],
      ...
      /**
       * Export all data (async, non-blocking)
       */
      exportAudit: async () => {
        await debouncedStorage.flush(); // Ensure all pending writes
        
        const state = get();
        const clientName = state.responses['nomination']?.value || 'SANS_NOM';
        const dateStr = new Date().toISOString().split('T')[0];
        
        // Collect all photo blobs asynchronously
        const photoExports = {};
        ...
        const dataToExport = {
          ...
          photos: photoExports,
          checksum: null // Could add integrity check
        };
      ...
      importAudit: async (file) => {
        ...
        try {
          ...
          // Version check
          if (!json.allSites) {
            // Legacy format - convert to multi-site
            const migrated = {
              allSites: {
                'Imported': {
                  name: 'Site Importé',
          ...
          // Modern format - restore photo blobs
          if (json.photos) {
      ...
      getStorageStats: async () => {
        ...
        const storeKeys = allKeys.filter(k => !k.startsWith('photo_'));
        
        let photoSize = 0;
        for (const key of photoKeys.slice(0, 10)) { // Sample first 10
          const blob = await get(key);
          if (blob) photoSize += blob.size;
        }
        ...
        return {
          ...
          estimatedPhotoSize: photoSize * (photoKeys.length / Math.min(photoKeys.length, 10))
        };
      }
    }),
    {
      name: 'risk-audit-storage-v2',
      storage: createJSONStorage(() => idbStorage),
      
      // Only persist these fields (lightweight)
      partialize: (state) => ({
        questionsConfig: state.questionsConfig,
        activeSiteId: state.activeSiteId,
        allSites: state.allSites,
        auditorInfo: state.auditorInfo,
        history: state.history.slice(0, 20), // Limit persisted history
        isLoaded: true
      }),
      ...
      onRehydrateStorage: () => (state, error) => {
        ...
        if (state) {
          ...
          // Sync current site data to compatibility state
...
// Use these in components instead of full store subscription
